...
| ------------------------------------------ | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [**forge**](./forge)                       | `/forge`              | Drives a PR from a one-line brief to **READY** through a proof chain — ground truth, goals, scenarios, tests, design, impl, lens-designed review, and continuous CI — attesting every step. No hard dependency — restack is configurable (`devloop`'s `/restack` recommended, or a built-in git fallback). |
...
| [**frugal**](./frugal)                     | `/frugal`             | Session-activatable cost optimization — decomposes work and routes each subtask to the cheapest adequate model+effort worker (haiku/sonnet × low..xhigh), ledger-tracked with recursion to depth 5; `/frugal-stats` reports estimated savings.                                                             |
