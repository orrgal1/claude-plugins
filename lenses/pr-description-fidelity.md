...
- Extract goal bullets from PR description verbatim.
...
Every other claim in the description matches the diff (no narrative drift).
...
- Backward-compat claims ("strictly additive") → spot-check at least one
  representative file's pre/post diff.
- Stack-position claims ("Base: PR #N", "PR-X of N-PR stack") → match
  `gh pr view` base ref + stack doc.
- **Drift → major** (not blocker, unless the drift hides a goal-miss). Author
  should refresh the description before merge so future readers and
  auto-generated changelogs aren't misled.
