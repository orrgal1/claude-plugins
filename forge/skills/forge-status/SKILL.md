...
| `AWAIT_AUTHOR_REVIEW`    | would-be READY, PR still `isDraft`, no `approvals.json.author_review`, not `--no-author-review`                                                                                                                                                                                | operator: `/forge approve` (self-review ingest) or `/forge iterate "<fbk>"`                           |
| `AWAIT_REVIEW_REQUEST`   | would-be READY, PR still `isDraft`, `approvals.json.author_review` present (or `--no-author-review`), no `approvals.json.review_request`, not `--no-review-request`                                                                                                            | operator: `/forge approve` (mark-ready+request) or `/request-review --ready`                          |
