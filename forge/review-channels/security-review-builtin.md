...
A focused security-vulnerability sweep alongside the lens fan-out — auth, input
handling, injection, secrets, dependency risk, crypto misuse, etc.

Shipped always-on (`default_enabled: true`). **Replaces** forge's former
...
**Dependency:** requires Claude Code's built-in `/security-review` skill. Forge
ships no fallback security lens — environments without that skill get no
security review unless the operator wires one (`.forge/lenses/security.md` + a
persona, or a custom channel).

## Selection

Wholesale — the wrapped skill picks its own scope from the current diff. No
further narrowing: security risk is rarely localized to a predictable file, so
the broad scan is the point.

Optional `--scope <path>` per-run flag passes through to restrict the scan.
...
- Each finding starts with a severity-labeled heading
  (`### [CRITICAL] / [HIGH] / [MEDIUM] / [LOW] / [INFO]` or equivalent
  emoji/bracket form).
- File + line follow on the next non-blank line as `file:line` (`file` alone
  when line-unresolved → `line: 0`).
- Body lines until the next severity heading or `---` separator.
- `Fix:` / `Remediation:` / `Suggested fix:` sub-block → `fix`. Absent →
  `fix: null`.
- Category from explicit tag (`[category: auth-bypass]`) or inferred from body's
  vulnerability-class keywords. Unclassifiable → `category: "unclassified"` +
  severity falls to `low` mapping (`minor`).
...
— security signal is too important to drop, so it lands as `major` (not `minor`
like other wrappers), pushing the operator to read `raw.md`.
...
**Cap discipline.** Ships `severity_cap: null` — critical/high findings pass
through as blockers. Repos preferring softer integration set
`[review.channels.security-review-builtin].severity_cap = "major"` so it never
blocks a READY verdict, only adds majors the green-loop chases.
